package com.bloodmangement.serviceAdmin.Helper;
...
public class ExcelHelper {
    ...
    public byte[]  getExcelFromListBloodRequest(List<BloodRequestProxy> bloodRequestList){

        try(ByteArrayOutputStream os = new ByteArrayOutputStream(); Workbook workbook = new XSSFWorkbook()){
            String Headers[] = {"ID","BLOOD GROUP","QUANTITY","REQUEST_DATE","STATUS"};
            ...
            //adding list data into other row
            int j=1;
            for(BloodRequestProxy request: bloodRequestList){
                Row row = sheet.createRow(j);
