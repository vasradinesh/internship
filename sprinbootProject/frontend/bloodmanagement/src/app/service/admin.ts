...
export class AdminService {
  ...
  approveDonation(id: number) {
    return this.http.get(`http://localhost:8080/admin/donor/${id}/approve`);
  }

  downloadBloodStockReport() {
    return this.http.get(
      'http://localhost:8080/admin/bloodstockreport/download',
      { responseType: 'blob' }
    );
  }
  ...
  downloadRequestReport() {
    return this.http.get(
      'http://localhost:8080/admin/blood/requestdata/download',
      { responseType: 'blob' }
    );
  }
